...
const DatePickerCustomInput = (
  {
    value,
    onClick,
  }: { value: string; onClick: React.MouseEventHandler<HTMLInputElement> },
  ref: React.Ref<HTMLInputElement>
) => <TextInput onClick={onClick} value={value} readOnly />;

export const NuevaCartaStandBy = () => {
  ...
  const [availableCompradores, setAvailableCompradores] = useState<
    Array<IComprador>
  >([]);
  ...
  const [
    addCarta,
    { data: responseData, isSuccess, isError, isLoading, error },
  ] = useAddCartaStandByMutation();
  ...
  useEffect(() => {
    if (watchEmpresa && catCompradores) {
      ...
      setAvailableCompradores(
        newCompradores
          .filter((c) => c.Activo)
          .filter((c) => c.EmpresaId === watchEmpresa)
      );
    }
  }, [watchEmpresa]);
  ...
  useEffect(() => {
    ...
    if (isSuccess) {
      if (
        responseData &&
        responseData.DataInt &&
        responseData.DataString &&
        responseData.DataString.length > 0
      ) {
        dispatch(
          addToast({
      ...
      <div className="md:grid md:grid-cols-12 md:gap-6 mb-6 px-6">
        <div className="md:col-span-3">
          ...
          <input
            type="hidden"
            value={18}
            {...register("TipoCartaId", { valueAsNumber: true })}
          />
        </div>
        ...
        <div className="md:col-span-3">
          ...
          <Select
            {...register("TipoStandBy")}
            color={`${formErrors.TipoStandBy && "failure"}`}
          >
        ...
        <div className="md:col-span-3">
          ...
          <Select
            ...
            color={`${formErrors.BancoId && "failure"}`}
          >
        ...
        <div className="md:col-span-3">
          ...
          <TextInput
            type="number"
            {...register("MontoOriginalLC", { valueAsNumber: true })}
          />
        </div>
            ...
            render={({ field }) => (
              <DatePicker
                customInput={React.createElement(
                  React.forwardRef(DatePickerCustomInput)
                )}
      ...
      <div className="bg-yellow-50 py-6 px-6 md:flex gap-6">
        <div className="mb-4 flex-1">
          ...
          <Textarea
            rows={5}
            className="text-sm"
            {...register("ConsideracionesReclamacion")}
          />
        </div>
        <div className="mb-4 flex-1">
          ...
          <Textarea
            rows={5}
            className="text-sm"
            {...register("ConsideracionesAdicionales")}
          />
        </div>
      </div>
